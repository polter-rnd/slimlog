...
void run_concurrent_test(int num_threads, int iterations, auto test_func)
{
...
const suite<char> Multithread("multithread", type_only, [](auto& _) {
    ...
    _.test("concurrent_logging", []() {
        ...
        auto log = std::make_shared<Logger<String, Char, MultiThreadedPolicy>>();
        log->template add_sink<OStreamSink>(std::cout);

        std::atomic<int> total_messages{0};

        // Temporarily disable console output
        const auto console_state = std::cout.rdstate();
        std::cout.setstate(std::ios_base::failbit);

        run_concurrent_test(num_threads, messages_per_thread, [&](int thread_id, int iteration) {
            const auto msg = from_utf8<Char>(
                "Thread " + std::to_string(thread_id) + " msg " + std::to_string(iteration));
            log->info(msg);
            total_messages.fetch_add(1, std::memory_order_relaxed);
        });

        // Restore console output
        std::cout.clear(console_state);
    ...
    _.test("concurrent_sink_management", []() {
        ...
        run_concurrent_test(num_threads, iterations_per_thread, [&](int thread_id, int iteration) {
    ...
    _.test("concurrent_level_changes", []() {
        ...
        expect(filtered_messages.load(), greater_equal(num_threads * iterations_per_thread));
    });
    ...
    _.test("concurrent_file_logging", []() {
        ...
        run_concurrent_test(num_threads, messages_per_thread, [&](int thread_id, int iteration) {
            const auto msg = from_utf8<Char>(
                "Thread " + std::to_string(thread_id) + " message " + std::to_string(iteration));
            log.info(msg);
    ...
    _.test("concurrent_hierarchy", []() {
        ...
        auto branch1_logger = std::make_shared<Logger<String, Char, MultiThreadedPolicy>>(
            root_logger, from_utf8<Char>("branch1"));
        auto branch2_logger = std::make_shared<Logger<String, Char, MultiThreadedPolicy>>(
        ...
        std::atomic<int> hierarchy_changes{0};
        ...
        run_concurrent_test(num_threads, iterations_per_thread, [&](int thread_id, int iteration) {
            if (thread_id % 3 == 0) {
                ...
                auto temp_child
                    = std::make_shared<Logger<String, Char, MultiThreadedPolicy>>(from_utf8<Char>(
                ...
                hierarchy_changes.fetch_add(1, std::memory_order_relaxed);
                ...
                if (persistent_children.empty()) {
                    for (int i = 0; i < 3; ++i) {
                        persistent_children.push_back(
                ...
                auto& child = persistent_children[iteration % persistent_children.size()];
                if (iteration % 4 == 0) {
                ...
                if (iteration % 10 == 0) {
                    // Occasionally restructure the main branches
                    if (iteration % 20 == 0) {
                ...
                auto temp1 = std::make_shared<Logger<String, Char, MultiThreadedPolicy>>(
                    from_utf8<Char>("chain1_" + std::to_string(thread_id)));
                auto temp2 = std::make_shared<Logger<String, Char, MultiThreadedPolicy>>(
                    temp1, from_utf8<Char>("chain2_" + std::to_string(thread_id)));
                auto temp3 = std::make_shared<Logger<String, Char, MultiThreadedPolicy>>(
                ...
                hierarchy_changes.fetch_add(
                    2, std::memory_order_relaxed); // Chain creation + modification
            }
        ...
        expect(hierarchy_changes.load(), greater(0));
        expect(messages_logged.load(), greater(0));

        // Verify final state is reasonable (no crashes during cleanup)
        expect(root_logger->parent(), equal_to(nullptr));
    });
    ...
    _.test("sink_parent_changes", []() {
        ...
        for (int iter = 0; iter < num_iterations; ++iter) {
            ...
            run_concurrent_test(
                num_threads, operations_per_thread, [&](int thread_id, int iteration) {
                    if (thread_id % 5 == 0) {
            ...
            expect(operations_completed.load(), equal_to(num_threads * operations_per_thread));
        }
    });
        ...
        for (int iter = 0; iter < num_iterations; ++iter) {
            ...
            run_concurrent_test(
                num_threads, operations_per_thread, [&](int thread_id, int iteration) {
                    ...
                    relationship_changes.fetch_add(1, std::memory_order_relaxed);
    ...
    _.test("stress_test", []() {
        ...
        for (int iter = 0; iter < num_iterations; ++iter) {
            ...
            run_concurrent_test(
                num_threads, operations_per_thread, [&](int thread_id, int iteration) {
                    std::mt19937 rng(thread_id * 1000 + iteration); // Deterministic but varied seed
            ...
            expect(total_operations.load(), equal_to(num_threads * operations_per_thread));
        }
    });
});
